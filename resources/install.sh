...
SOURCE_BINARY="${SCRIPT_DIR}/bin/${BINARY_NAME}"
...
if [ ! -f "$SOURCE_BINARY" ]; then
    error "Could not find binary for ${OS}/${ARCH} at ${SOURCE_BINARY}"
fi

echo "Installing git-ai from ${SOURCE_BINARY}..."
cp -f "$SOURCE_BINARY" "${INSTALL_DIR}/git-ai"

# Verify checksum if embedded (release builds only)
# Checksum verification skipped for local install
