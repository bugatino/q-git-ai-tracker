    ...
    return new Promise((resolve) => {
        ...
        proc.on('close', (code) => {
            if (code === 0) {
                ...
                vscode.window.showInformationMessage(`✅ Git-ai checkpointed Amazon Q → ${range.end.line - range.start.line + 1} lines`, { modal: false });
            }
            else {
...
function updateStatusBar(editor) {
    const recentCount = recentChanges.filter(c => Date.now() - c.timestamp < 5 * 60 * 1000 // 5 phút gần nhất
    ).length;
